...
import { DollarSign, Eye, Pencil, X } from "lucide-react";
...
const ListaPedidosUsuarios = () => {
  const pedidos = [
    ...
    },
    {
      ...
      total: 200.0,
      status: "Pagado",
      details: [{ name: "Producto 3", quantity: 4, price: 50.0 }],
          ...
          <Tbody>
            {pedidos.map((pedido, index) => (
              <Tr key={pedido.id}>
                ...
                <Td textAlign="center">
                  {pedido.status === "Pendiente" ? remainingTimes[index] : "-"}
                </Td>
                <Td textAlign="center">
                  <Stack direction="row" spacing={2} justify="center">
                    <Tooltip
                      label="Mostrar detalles del pedido"
                      aria-label="Mostrar detalles del pedido"
                    >
                      <IconButton
                        icon={<Eye />}
                        size="sm"
                        onClick={() =>
                          console.log("Mostrar detalles del pedido", pedido)
                        }
                        colorScheme={"green"}
                      />
                    </Tooltip>
                    <Tooltip label="Editar pedido" aria-label="Editar pedido">
                      <IconButton
                        ...
                        colorScheme={
                          pedido.status !== "Pendiente" ? "gray" : "green"
                        }
                      />
                    </Tooltip>
                    <Tooltip
